...
GALAXY_PARAMS = {
    "PGC": {
        ...
        # --- границы физических величин = края сетки E-MILES (гайд §13) ---
        "age_min":      0.1,
        "age_max":      14.1,
        "met_min":      -1.71,
        "met_max":      0.22,
    ...
    "LEDA": {
        # --- численные параметры модели ---
        "scale":        32,
        # ВНИМАНИЕ: "incl" — НЕ наклон галактики. Истинный наклон модели i=27°
        # (имя архива ..._i27_..., forstand_C INCL=27). Это угол поворота для
        # rotation_matrix_x в orbit_map: a=0°→edge-on, a=90°→face-on,
        # поэтому a = 90° − i = 90 − 27 = 63°.
        "incl":         63,           # угол поворота orbit_map = 90° − i(=27°); НЕ наклон
        "model_index":  5,            # финал C (приор v=125): Υ=4.781, индекс 5 из 9
        "gamma":        225,

        # --- границы BVLS = диапазон наблюд. карт ± малый запас (НЕ края E-MILES) ---
        # данные LEDA: age [1.48, 5.18], [M/H] [-0.87, 0.14]
        "age_min":      1.0,
        "age_max":      5.5,
        "met_min":      -1.0,
        "met_max":      0.3,
        "lambda_reg":   0.01,         # knee χ²_red-скана (как PGC)

        # --- файлы данных (финал C, приор v_halo=125, 20k орбит) ---
        "npz_file":     "M1e+07_O0_Rh30.6_Vh125_i27_a0_N20000_R1.00_GH_DensityCylindricalLinear.npz",
        "bins_file":    "bins_LEDA_2220522_Damir's.txt",
        "chem_file":    "results_8254-1902_vorb020_md19_ad-1_nmom4.fits",
        "orbits_dir":   "C:/Users/Кель Ыр/Desktop/Галактики/Статья-2/LEDA/results_C/orbits_M1e+07_O0_Rh30.6_Vh125_i27_a0_N2000",  # абс. путь (2 ГБ, не копировал)
