...
M_total = 4.5e9  # светимость-нормировка подписи колорбара (×Υ = масса)
...
agama_alpha = 0.0    # ALPHA из run-скрипта (0 для PGC)
agama_beta  = 42.0   # INCL из run-скрипта (42° для PGC, наклонение)
agama_gamma = 0.0    # gamma1 из run-скрипта (PA на небе, 0° для PGC).
...
gamma = -0  # Угол для transform кинематики (исторический, оставлен для совместимости)

# Угол поворота МОДЕЛЬНОЙ ПЛОТНОСТИ относительно горизонтали.
# По смыслу — позиционный угол PA галактики на небе (gamma1 в углах Эйлера
# Agama при построении модели).
#
# Значение берётся из run-скрипта (gamma1 в градусах). Примеры:
#   PGC 35706:    gamma1 = 0°  → density_PA = 0.0
#   LEDA 2220522: gamma1 = 40° → density_PA = 40.0
density_PA = 0.0

# Преобразования модельной плотности для согласования с кинематикой.
# По умолчанию — транспозиция (.T), та же операция, что применяется к
# модельной кинематике через flip_diagonal_before_transform=True.
#
# Причина: кинематика и плотность строятся в разных проекциях:
#   - кинематика — через bin_scheme (наблюдательная система MaNGA),
#     к которой потом применяется .T;
#   - плотность — через траектории, в прямой Agama-проекции
#     (alpha=0, beta=42°, gamma=0 → большая ось горизонтально).
# bin_scheme приходит транспонированным относительно стандарта Agama,
# и авторы кода компенсировали это .T для кинематики, но забыли
# применить ту же операцию к плотности.
...
CACHE_DIR_ORIGINAL = "density_cache_original_PGC_agama_21r"  # проекция (X, Y)
CACHE_DIR_ROTATED = "density_cache_rotated_PGC_agama_21r"   # проекция (X, Y)
...
def tickers_X_formatter(x, pos):
    """Форматтер для подписей в угловых секундах (с шагом 6)"""
...
def tickers_Y_formatter(y, pos):
    """Форматтер для подписей в угловых секундах по Y (с шагом 6)"""
...
def orbit_map_original(orb_groups, incl):
    ...
    # сторону к rotation_matrix_x (agama beta=90 → (x,z,-y); rot_x(90) → (x,-z,y)).
    # Поэтому incl_extra нужно инвертировать по знаку: иначе col1 (incl=90) даёт
    # эффективный наклон ≈6° (почти face-on), а не edge-on. Проверено probe-скриптом
    # (minor/major: было 0.97≈face-on, стало 0.60=истинное ребро). col0 (incl_extra=0)
    # не затрагивается.
    ...
    # Азимутальное усреднение: модель осесимметрична, поэтому траектория каждой
    # орбиты размазывается по азимуту φ (вокруг оси симметрии z в galactic frame).
    # Без этого редконаселённые-по-массе компоненты (противовращ. диск, N_eff~60)
    # дают «шестерёнку» — прямой отпечаток нескольких тяжёлых резонансных орбит.
    # Векторизовано по точкам ради скорости.
    N_AZ = 24
    ...
    for orb_group in orb_groups:
        for orbi in orb_group:
            ...
            try:
                with open(f"orbits_M1e+07_O0_Rh71.6_Vh162_i42_a0_N2000/orbit_{orbi}.txt", "r") as f:
            ...
            if pts.ndim != 2 or pts.shape[0] == 0:
                continue
            # цилиндрические (R, φ0, z) в galactic frame и размазка по φ
            ...
            sky = M @ np.vstack([xx, yy, zz])      # (2, Npts*N_AZ)
            ...
            try:
                ...
                with open(f"orbits_M1e+07_O0_Rh71.6_Vh162_i42_a0_N2000/orbit_{orbi}.txt", "r") as f:
                    for str_point in f:
                        ...
                        # ИЗМЕНЕНО: проекция на плоскость неба (X, Y).
...
class FormattedPlotCreator:
    ...
    def __init__(self):
        ...
        # Поворот плотности модели на PA (gamma в углах Эйлера Agama).
        # Для PGC 35706: alpha=0, beta=42°, gamma=0 → density_PA = 0.
        ...
        self.norm_photometry = colors.LogNorm(vmin=Phot_min, vmax=np.max(phot['CCD4'].data))
    ...
    def create_formatted_plots(self, use_rotated_plane=False, rotate_kinematics=True, 
        ...
        filename = f"losvd_and_surface_density_PGC_3570{filename_suffix}.pdf"
    ...
    def _plot_photometry(self, ax, fig):
        ...
        # ИЗМЕНЕНО: Используем CCD4 из второго кода
        ...
        ax.yaxis.set_major_locator(tcr.IndexLocator(12, offset=-8.5))
        ax.xaxis.set_major_locator(tcr.IndexLocator(12, offset=-8.5))
        ...
        ax.yaxis.set_minor_locator(tcr.IndexLocator(2, offset=-2.5))
        ax.xaxis.set_minor_locator(tcr.IndexLocator(2, offset=-2.5))
        ...
        ax.annotate(r"$Log_{10}(Поток)[отсчет/пиксель]$", (3, 93),  
    ...
    def _plot_observational_velocity(self, ax, data, fig, rotate_kinematics=True):
        ...
        ax.annotate(r" $V_{0}[км/с]$", (1, 34), fontsize=6, style="italic")
        
        ax.yaxis.set_major_formatter(tcr.NullFormatter())
        ax.xaxis.set_major_formatter(tcr.NullFormatter())
        # ИЗМЕНЕНО: Offsets из второго кода (-3.5 вместо -7.5)
        ax.yaxis.set_major_locator(tcr.IndexLocator(12, offset=-3.5))
        ax.xaxis.set_major_locator(tcr.IndexLocator(12, offset=-3.5))
        ax.yaxis.set_minor_formatter(tcr.NullFormatter())
        ax.xaxis.set_minor_formatter(tcr.NullFormatter())
        ...
        major_locs = np.arange(-12, 13, 6) * 2 + scale/2  # Из второго кода: 12 шаг
    ...
    def _plot_observational_sigma(self, ax, data, fig, rotate_kinematics=True):
        ...
        # ИЗМЕНЕНО: Позиция поправлена для scale=42 (было 1,26)
        ax.annotate(r" $\sigma[км/с]$", (1, 34), fontsize=6, style="italic")
    ...
    def _plot_component_row(self, axs, row, lambda_z_range, bounds, component_name, 
        ...
        if row == 1:
            ...
            axs[2, 0].set_title(f"PGC 35706", fontdict=fontdict, pad=100)  # Из второго кода
            
            # ИЗМЕНЕНО: Позиция поправлена для scale=42 (было 1,28)
            axs[row, 0].annotate(r"$Log_{10}(\mu_{b})[M_{\odot}/пиксель]$", (1, 37),
        ...
        if row == 1:
            ...
            axs[row, 0].annotate(r"$i =$" + f"{displayed_inclination_col0}" + r"$\degree$", (1, 30), fontsize=6,
                               path_effects=[pe.withStroke(linewidth=1, foreground="white")])
        else:
            # ИЗМЕНЕНО: Позиция поправлена для scale=42 (было 1,28)
            axs[row, 0].annotate(r"$i =$" + f"{displayed_inclination_col0}" + r"$\degree$", (1, 37), fontsize=6,
                               path_effects=[pe.withStroke(linewidth=1, foreground="white")])
    ...
    def _configure_density_axes(self, ax, row, col, show_title=False):
        ...
        ax.yaxis.set_major_locator(tcr.IndexLocator(12, offset=offset))
        ax.xaxis.set_major_locator(tcr.IndexLocator(12, offset=offset))
        ax.yaxis.set_minor_formatter(tcr.NullFormatter())
        ax.xaxis.set_minor_formatter(tcr.NullFormatter())
        ax.yaxis.set_minor_locator(tcr.IndexLocator(2, offset=-2.5))  # Из второго кода
        ax.xaxis.set_minor_locator(tcr.IndexLocator(2, offset=-2.5))  # Из второго кода
        # ИЗМЕНЕНО: уменьшен размер шрифта с 8 до 6
        ax.tick_params(direction="in", which="major", width=0.3, length=2, labelsize=6)
        ax.tick_params(direction="in", which="minor", width=0.15, length=1)
