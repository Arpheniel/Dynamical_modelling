#!/bin/bash
# launch_C_LEDA.sh -- APPROACH C (1-D mass-normalization scan at a
# cosmologically fixed NFW scale radius) for LEDA 2220522.
# Place next to forstand_C_LEDA_2220522.py and the galaxy's data files.
# Run:  bash launch_C_LEDA.sh
#
...
#   logs/chunkC*_LEDA.log         -- per-worker stdout/stderr
#   logs/postgridC_LEDA.log       -- postgrid stdout/stderr
#
# Auto-resume:
#   If killed mid-scan, just rerun `bash launch_C_LEDA.sh`. Per-cell
...
if [ -z "$PY" ]; then
    echo "ERROR: no forstand_C_LEDA_*.py in current folder ($(pwd))"
    exit 1
fi
...
if [ $N_FAILED -gt 0 ]; then
    ...
    echo "  See logs/chunkC*_LEDA.log for details."
    echo "  Re-run 'bash launch_C_LEDA.sh' to auto-resume."
    echo "============================================================"
    exit 1
fi
...
python -u "$PY" phase=postgrid nchunks=$NCHUNKS 2>&1 | tee logs/postgridC_LEDA.log
