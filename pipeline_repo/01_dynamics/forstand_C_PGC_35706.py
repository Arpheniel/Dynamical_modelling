#!/usr/bin/python
'''
forstand_C_PGC_35706.py
=================================
APPROACH C = 1-D mass-normalization scan at a COSMOLOGICALLY FIXED NFW
scale radius.  Sibling of forstand_grid_R1_PGC_35706.py (the 2-D
...
---------------------------
    ...
    files; density type (DensitySphHarm for PGC); inclination/PA;
    distance; kinematic data; Voronoi binning; Mbh=1e7; subprocess
    isolation + 3-attempt retry resilience.  The steep central MGE
    component of PGC (sigma=0.23", unresolved by 1" PSF) still leaves a
    residual systematic in the honest chi^2 -- approach C does not
    address that, only the halo dimensionality.
...
import os, sys, glob, subprocess, tempfile, time, numpy, agama
...
# h-correction below converts it to physical Msun.  PGC 35706: logM* = 10.439.
LOGMSTAR_CAT = float(args.get('LOGMSTAR_CAT', 10.439))
...
def lnprob_fun(pars0, bake_upsilon=1.0):
    ...
    # See forstand_grid_R1_LEDA_2220522.py header for full discussion.
    if baked:
        mge = mge.copy()
        mge[:, 0] *= bake_upsilon
    ...
    densityParams = dict(type=(
        ...
        'DensityCylindricalTopHat','DensityCylindricalLinear')[2])
...
def _run_cell_in_subprocess(rhalo_val, vhalo_val, max_attempts=3):
    ...
    Background: random python-process crashes have been observed on this
    server; spacing retries gives the OS / NFS / shared memory a chance
    to settle.
...
if phase == 'all':
    ...
    save_orb       = True
    save_orbits_to = "/data1/vgorad/dynam_mod/Dynamical_modelling/Gal/chemo-dynamical_modeling/PGC_35706"
    lnprob_fun(bestfit)
    print('\nDone.')
    ...
    try:
        ...
        # Python-level exception: write NaN, exit 0.  Parent treats this as
        # "completed but NaN", does not retry.
        print('  [onecell] Python exception: %s' % e, flush=True)
