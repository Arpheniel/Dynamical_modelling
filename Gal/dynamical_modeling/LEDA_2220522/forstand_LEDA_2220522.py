...
nbody     = int  (args.get('NBODY', 10000000))    # [OPT] number of particles for the N-body representation of the best-fit model
...
if command == 'RUN':
    ...
    #####
    xlim = (min([numpy.amin(p[:,0]) for p in apertures1]), max([numpy.amax(p[:,0]) for p in apertures1]))*10
    ylim = (min([numpy.amin(p[:,1]) for p in apertures1]), max([numpy.amax(p[:,1]) for p in apertures1]))*10
    print(xlim,ylim)
    gridmax = max(abs(xlim[0]), abs(xlim[1]), abs(ylim[0]), abs(ylim[1]))
    gridmin = gridmax * 1e-3
    gridpix = agama.symmetricGrid(101, gridmin, gridmax)
    plot_gridx = gridpix[(gridpix>=xlim[0]) * (gridpix<=xlim[1])]
    plot_gridy = gridpix[(gridpix>=ylim[0]) * (gridpix<=ylim[1])]
    projectedDensityMag = numpy.zeros((len(plot_gridx), len(plot_gridy)))
    print(projectedDensityMag.shape)
    for d in datasets:
        if hasattr(d, 'projectedDensity'):
            ...
            projectedDensityMag = -2.5 * numpy.log10(projectedDensity / numpy.max(projectedDensity)) - 0.01
            numpy.savetxt('projectedDensity.txt',projectedDensityMag.T)
    ...
    agama.schwarzlib.runPlot(datasets=datasets, aval=Mbh, bval=ML, chi2=chi2, filenames=filenames,
        ...
        v0lim=(-50,50), sigmalim=(40,160), v0err=15.0, sigmaerr=15.0, potential=pot_total)
