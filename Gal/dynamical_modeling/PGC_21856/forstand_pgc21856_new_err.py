...
distance  = float(args.get('DISTANCE', 90000))# [REQ] assumed distance [kpc]
...
vhalo     = float(args.get('VHALO', 180))       # [OPT] asymptotic (LOG) or peak (NFW) circular velocity of the halo [km/s]
rhalo     = float(args.get('RHALO', 150))       # [OPT] core (LOG) or scale (NFW) radius of the halo [lenth_unit]
Upsilon   = float(args.get('UPSILON', 8))     # [OPT] initial value of mass-to-light ratio in the search
...
regul     = float(args.get('REGUL', 2. ))       # [OPT] regularization parameter (larger => more uniform orbit weight distribution in models)
incl      = float(args.get('INCL', 75.0))       # [REQ] inclination angle (0 is face-on, 90 is edge-on) [degrees]
...
numpy.set_printoptions(precision=10, linewidth=9999, suppress=True)
...
filenameMGE = 'mge_PGC_21856_z_legacy.txt'    # [REQ] file with parameters of the MGE model for the surface density profile (if MGE is used)

### common parameters for kinematic datasets (though in principle they may also differ between them)
gridv  = numpy.linspace(-500, 500, 46)  # [REQ] the grid in model velocity space (will be multiplied by sqrt(Upsilon) when comparing to data)
...
hist_degree = 1               # [OPT] B-spline degree for the observed LOSVDs (0 means histogram)
hist_gridv  = numpy.linspace(-980, 980, 28)  # [OPT] velocity grid for the observed LOSVDs (boundaries of velocity bins, not centers!)

### parameters of the 1st kinematic dataset
gamma1 = 30 * numpy.pi/180  # [REQ] CW rotation angle of the image-plane X axis relative to the line of nodes (=major axis for axisym.systems)
...
filenameVorBin1 = 'bins_HIST_pgc21856.txt' # [REQ] Voronoi binning scheme for this dataset
filenameHist1   = 'Kinem_hist_pgc21856.txt'  # [REQ] histogrammed representation of observed LOSVDs
filenameGH1     = 'kinem_gh_SOME.txt'# [REQ] Gauss-Hermite parametrization of observed LOSVDs (usually only one of these two
"""
### same for the 2nd kinematic dataset [OPT] - may have only one dataset, or as many as needed
gamma2 = 25.0 * numpy.pi/180
...
filenameVorBin2 = 'bins_PGC21856.txt'
filenameHist2   = 'kinem_hist_i%.0f_hr.txt'   % incl
filenameGH2     = 'kinem_gh_PGC21856.txt'
"""
...
if command == 'MOCK':
    ...
    hist_err = (numpy.maximum(hist_val, particlemass) * particlemass)**0.5  # again estimate errors from Poisson noi

    print(numpy.dstack((hist_val, hist_err)).reshape(len(apertures), -1),hist_val,hist_err)
...
densityParams = dict(type = (
    ...
    'DensityCylindricalLinear')[3])   # [REQ] choose one of these types!
...
def process_errors(errors):
    ...
    # Заменяем нулевые ошибки на 0.002
    errors[errors == 0] = 0.1
    # Умножаем маленькие ошибки (<0.001) на 2
    errors[errors < 0.002] *= 10
    return errors
### ### ### 
...
if usehist:
    ...
    # Обрабатываем ошибки
    obs_err = process_errors(kindat1[:, 1::2])
    datasets.append(agama.schwarzlib.KinemDatasetHist(
        ...
        obs_err   = obs_err,           # [MOD] processed errors
...
if command == 'RUN':
    ...
    print(float(Omega),float(rhalo),float(vhalo),float(incl),float(alpha_deg),int(numOrbits),float(regul))
    ...
    agama.schwarzlib.runPlot(datasets=datasets, aval=Mbh, bval=ML, chi2=chi2, filenames=filenames,
        ...
        v0lim=(-150,150), sigmalim=(40,260), v0err=15.0, sigmaerr=15.0, potential=pot_total)
