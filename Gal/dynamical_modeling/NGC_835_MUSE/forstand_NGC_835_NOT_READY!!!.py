...
distance  = float(args.get('DISTANCE', 24000))# [REQ] assumed distance [kpc]
...
vhalo     = float(args.get('VHALO', 210))       # [OPT] asymptotic (LOG) or peak (NFW) circular velocity of the halo [km/s]
rhalo     = float(args.get('RHALO', 120))       # [OPT] core (LOG) or scale (NFW) radius of the halo [lenth_unit]
...
incl      = float(args.get('INCL', 60))       # [REQ] inclination angle (0 is face-on, 90 is edge-on) [degrees]
...
filenameMGE = 'mge_NGC_835_z_legacy.txt'    # [REQ] file with parameters of the MGE model for the surface density profile (if MGE is used)
...
gamma1 = 131 * numpy.pi/180  # [REQ] CW rotation angle of the image-plane X axis relative to the line of nodes (=major axis for axisym.systems)
...
filenameVorBin1 = 'bins_NGC_835.txt' # [REQ] Voronoi binning scheme for this dataset
filenameHist1   = 'kinem_hist_lr.txt'    # [REQ] histogrammed representation of observed LOSVDs
filenameGH1     = 'kinem_gh_NGC_835.txt'# [REQ] Gauss-Hermite parametrization of observed LOSVDs (usually only one of these two
...
filenameVorBin2 = 'bins_NGC_524.txt'
filenameHist2   = 'kinem_hist_i%.0f_hr.txt'   % incl
filenameGH2     = 'kinem_gh_NGC_524.txt'
"""
...
if command == 'RUN':
    ...
    if not usehist:
        ...
        #Sax[3].scatter(aperture_radii2, kindat2[:,2],      c='c', marker='x')
    ...
    agama.schwarzlib.runPlot(datasets=datasets, aval=Mbh, bval=ML, chi2=chi2, filenames=filenames,
        ...
        v0lim=(-200,200), sigmalim=(40,300), v0err=15.0, sigmaerr=15.0, potential=pot_total)
    
    mat_kin = agama.ghMoments(degree=degree, gridv=gridv * Upsilon**0.5,
