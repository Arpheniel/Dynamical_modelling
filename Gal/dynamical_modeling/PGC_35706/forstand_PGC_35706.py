...
vhalo     = float(args.get('VHALO', 185))       # [OPT] asymptotic (LOG) or peak (NFW) circular velocity of the halo [km/s]
rhalo     = float(args.get('RHALO', 169))       # [OPT] core (LOG) or scale (NFW) radius of the halo [lenth_unit]
Upsilon   = float(args.get('UPSILON', 15.3))     # [OPT] initial value of mass-to-light ratio in the search
...
if command == 'RUN':
    ...
    agama.schwarzlib.runPlot(datasets=datasets, aval=Mbh, bval=ML, chi2=chi2, filenames=filenames,
        ...
        v0lim=(-150,150), sigmalim=(40,200), v0err=15.0, sigmaerr=15.0, potential=pot_total)
