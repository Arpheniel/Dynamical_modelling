...
scale = 32
N = 40000  # number of orbits
M_total = 4.5e9  # in units of M_sun
distance = 108643000# in [pc]
incl = 63# Из второго кода
model_index = 8  # Из второго кода
gamma = 225  # Химия отзеркалена по оси у
...
# ИЗМЕНЕНО: Файлы спектра, фотометрии, bin_scheme и химии из второго кода
...
chem = fits.open("results_8254-1902_vorb020_md19_ad-1_nmom4.fits")
    ...
    for orb_group in orb_groups:
        for orbi in orb_group:
            ...
            try:
                ...
                with open(f"orbits_M1e+07_O0_Rh131_Vh324_i27_a0_N40000/orbit_{orbi}.txt", "r") as f:
                    for str_point in f:
...
def create_weight_matrix_of_dyn_comps(lambda_reg):
    ...
    # ==========
    # Создание матрицы N_bins х N_comps (матрица весов) содержащую инфорамию о вкладе каждого компонента
...
def main():
    ...
    weight_matrix, weight_map_binned = create_weight_matrix_of_dyn_comps(lambda_reg = 0.05)
