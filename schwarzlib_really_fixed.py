...
def runModel(datasets, potential, ic, Omega=0, intTime=100.0,
    ...
    for iorb, orb in enumerate(trajs):
        ...
        # FIX: убраны ошибочные перекрёстные члены v_x v_y, v_x v_z, v_y v_z;
        # MOD_V2 — это RMS-скорость sqrt(<v_x^2 + v_y^2 + v_z^2>), та же величина, что и MOD_VEL ниже
        MOD_VEL = _numpy.sqrt(_numpy.mean(t[:,3]**2 + t[:,4]**2 + t[:,5]**2))
        MOD_R = _numpy.sqrt(_numpy.mean(t[:,0]**2 + t[:,1]**2 + t[:,2]**2))
        MOD_Lambda_z = Lz[iorb] / (MOD_VEL * MOD_R)
    ...
    def solve(Upsilon):
        ...
        for orbi in range(len(Rm)):  # FIX: было range(0,len(Rm)-1) — пропускалась последняя орбита
