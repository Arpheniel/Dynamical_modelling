...
class KinemDatasetHist:
    ...
    def __init__(self, density, obs_degree, obs_gridv, obs_val, obs_err, tolerance=0, **target_params):
        ...
        #print('self.obs_val', len(self.obs_val), _numpy.max(self.obs_val), _numpy.min(self.obs_val), _numpy.median(self.obs_val), _numpy.mean(self.obs_val))
        #print('self.obs_err', len(self.obs_err), _numpy.max(self.obs_err), _numpy.min(self.obs_err), _numpy.median(self.obs_err), _numpy.mean(self.obs_err))
        #print('self.cons_val', len(self.cons_val), _numpy.max(self.cons_val), _numpy.min(self.cons_val), _numpy.median(self.cons_val), _numpy.mean(self.cons_val))
        #print('self.cons_err', len(self.cons_err), _numpy.max(self.cons_err), _numpy.min(self.cons_err), _numpy.median(self.cons_err), _numpy.mean(self.cons_err))
        """
        for i in range(0,len(self.cons_err)):
            if self.cons_err[i] < 0.001:
                self.cons_err[i] = _numpy.median(self.cons_val)*0.1
        """
        #print('self.cons_err NEW', len(self.cons_err), _numpy.max(self.cons_err), _numpy.min(self.cons_err), _numpy.median(self.cons_err), _numpy.mean(self.cons_err))
    ...
    def getGHMoments(self, LOSVD=None, Upsilon=None):
        ...
        '''
        ghorder = 6
        ind = tuple((1,2,6,7,8,9))  # indices of columns containing v,sigma,h3...hM in the matrix returned by ghMoments
        if LOSVD is None:
...
def runModel(datasets, potential, ic, Omega=0, intTime=100.0,
    ...
    filePrefix='', linePrefix='', fileResult='results.dat', nbody=False, nbodyFormat='text',save_orbits=False,save_orbits_to = ''):
    '''
    ...
    ############################################################################
    #print(trajs)
    #print("##############################",len(trajs),len(matrices[0]))
    #print(matrices)
    #trajs_MOD_LOSVD = trajs
    #matrices_MOD_LOSVD = matrices
    MOD_LOSVD_MULT = _numpy.zeros((1,len(trajs)))[0]
    #print(MOD_LOSVD_MULT)
    ###############################################################################################
    lambda_z = []
    MOD_VELOCITY = []
    MOD_VEL_X0 = []
    ...
    for iorb, orb in enumerate(trajs):
        ...
        MOD_V2 = _numpy.sqrt(_numpy.mean((t[:,3]**2 + t[:,4]**2 + t[:,5]**2 + 2 * t[:,3] * t[:,4] + 2 * t[:,3] * t[:,5] + 2 * t[:,4] * t[:,5])))
        MOD_R = _numpy.sqrt(_numpy.mean(t[:,0]**2 + t[:,1]**2 + t[:,2]**2))
        MOD_Lambda_z = Lz[iorb]/(MOD_V2 * MOD_R)
        
        
        #if MOD_Lambda_z > 1 or MOD_Lambda_z < -1:
           # print(Lz[iorb],MOD_V2*MOD_R,iorb,_numpy.mean(t[:,0]*t[:,4]),_numpy.mean(t[:,1]*t[:,3]))
            #print(Rm[iorb],_numpy.mean(t[:,3]),_numpy.mean(t[:,1]),_numpy.mean(t[:,4]),_numpy.mean(t[:,0]))
        
        
        lambda_z.append(MOD_Lambda_z)
        MOD_VEL = _numpy.sqrt(_numpy.mean(t[:,3]**2 + t[:,4]**2 + t[:,5]**2))
        MOD_VELOCITY.append(MOD_VEL)
    ...
    #######################################
    """
    for i_MOD_LOSVD in (0,len(trajs)-1):
        if Ci[i_MOD_LOSVD] < -0.8:
            MOD_LOSVD_MULT[i_MOD_LOSVD] = 1
    """
    #######################################
    ...
    def solve(Upsilon):
        ...
        #for ds in datasets:
        #    print('ds.cons_err STAT', _numpy.max(ds.cons_err), _numpy.min(ds.cons_err), _numpy.median(ds.cons_err), _numpy.mean(ds.cons_err))
        ...
        #########################
        MOD_weights_mult_matrices = _numpy.zeros((21,21),dtype=object)
        for i in range(21):
            for j in range(21):
        ...
        """
        weights_MOD_LOSVD = weights  * MOD_LOSVD_MULT
        #print(weights_MOD_LOSVD,weights,MOD_LOSVD_MULT)
        """
        #########################
        ...
        ####################
        """
        superpositions_MOD_LOSVD = [ weights_MOD_LOSVD.dot(mat) for mat in matrices ]
        """
        ####################
        ...
        DYN_COMP_LOSVD_temp = _numpy.zeros((21,21),dtype = object)
        
        for lambda_z_i in range(21):
            for R_mean_i in range(21):
                superpositions_dyn_comp = [ MOD_weights_mult_matrices[lambda_z_i][R_mean_i].dot(mat) for mat in matrices ]
                for ds, pen, sup in zip(datasets, penalties, superpositions_dyn_comp):
                    if hasattr(ds, 'mod_gridv'):  # kinematic dataset
                        DYN_COMP_LOSVD_temp[lambda_z_i][R_mean_i] = (sup.reshape(ds.num_aper, ds.num_bsplines).astype(_numpy.float32))            
        
        archive['DYN_COMP_LOSVD'].append(DYN_COMP_LOSVD_temp)
        ...
        for ds, pen, sup in zip(datasets, penalties, superpositions):
            ...
            if hasattr(ds, 'mod_gridv'):  # kinematic dataset
                print(ds.num_aper,ds.num_bsplines)
...
def runPlot(datasets,                           # list of [kinematic] datasets to be plotted
    ...
    def plotOrbit(indOrbit):
        ...
        this.oro.set_data([this.sco.get_offsets()[indSel][0]],[this.sco.get_offsets()[indSel][1]])
        this.orp.set_data([this.scp.get_offsets()[indSel][0]],[this.scp.get_offsets()[indSel][1]])
