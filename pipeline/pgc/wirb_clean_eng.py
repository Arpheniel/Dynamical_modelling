...
counter_rot_end_idx = 8    # контр-вращение: range(0, counter_rot_end_idx)
co_rot_start_idx    = 13   # со-вращение:    range(co_rot_start_idx, 21)
...
def orbit_map_original(orb_groups, incl):
    ...
    incl_extra = incl - real_inclination
            ...
            try:
                with open(f"orbits_M1e+07_O0_Rh107_Vh154_i42_a0_N40000/orbit_{orbi}.txt", "r") as f:
                    for str_point in f:
                        ...
                        # Проекция через Agama-матрицу + доворот камеры.
                        x_sky, y_sky = M @ position

                        x_ = int(x_sky + scale/2)
                        y_ = int(y_sky + scale/2)
...
class FormattedPlotCreator:
    ...
    def create_formatted_plots(self, use_rotated_plane=False, rotate_kinematics=True, 
        ...
        fig.text(0.19, 0.5, 'arcsec', va='center', rotation='vertical', fontsize=7)
        fig.text(0.83, 0.5, 'arcsec', va='center', rotation='vertical', fontsize=7)
        fig.text(0.5, 0.07, '    arcsec', ha='center', fontsize=7)
        ...
        self._plot_component_row(axs, 1, 
                               ...
                               component_name="Co-rotating disk", 
        ...
        self._plot_component_row(axs, 2, 
                               ...
                               component_name="Counter-rotating disk",
                               use_rotated_plane=use_rotated_plane, 
                               plane_rotation=plane_rotation,
                               rotate_kinematics=rotate_kinematics,
                               flip_diagonal=flip_diagonal_before_transform,
    ...
    def _plot_photometry(self, ax, fig):
        ...
        ax.annotate(r"$Log_{10}(Flux)[counts/pixel]$", (3, 93),  
    ...
    def _plot_observational_velocity(self, ax, data, fig, rotate_kinematics=True):
        ...
        ax.annotate(r" $V_{0}[km/s]$", (1, 34), fontsize=6, style="italic")
    ...
    def _plot_observational_sigma(self, ax, data, fig, rotate_kinematics=True):
        ...
        # ИЗМЕНЕНО: Позиция поправлена для scale=42 (было 1,26)
        ax.annotate(r" $\sigma[km/s]$", (1, 34), fontsize=6, style="italic")
    ...
    def _plot_component_row(self, axs, row, lambda_z_range, bounds, component_name, 
        ...
        if row == 1:
            ...
            axs[row, 0].annotate(r"$Log_{10}(\mu_{b})[M_{\odot}/pixel]$", (1, 37),
