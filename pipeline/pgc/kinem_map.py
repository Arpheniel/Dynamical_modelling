...
def setup_axis(ax, show_y=False, show_x=False, is_phot=False, offset_x=0, offset_y=0):
    ...
    if show_y:
        ...
        if is_phot:
            ax.yaxis.set_major_locator(tcr.IndexLocator(5, offset=2.5))
        else:
...
def plot_comparison_maps():
    # Загрузка фотометрических данных
    phot_obs = np.loadtxt('converted_fits_PGC_35706_z_OLD.txt')
    phot_model = np.loadtxt('model_PGC_35706_z.txt')
    phot_res = np.abs(phot_obs - phot_model)
    
    # Исправление: замена None на np.nan и маскирование
    ...
    Vel_min, Vel_max = -80, 80
    Sig_min, Sig_max = 0, 200
    ...
    ax1.annotate(r"$Log_{10}(Поток)[отсч./пикс.]$", (1, 23), 
                fontsize=7, style="italic")
    ax1.set_title("набл.", fontsize=19)
    ...
    ax2.set_title("модель", fontsize=19)
    ...
    ax3.annotate(galaxy_name, (14, 23), fontsize=11, style="italic")
    ax3.set_title("невязка", fontsize=19)
    ...
    ax4.set_ylabel("угл. сек.", fontsize=11)
    ax4.annotate(r"$V_{0}[км/с]$", (2, 37), fontsize=11, style="italic")
    ...
    ax7.annotate(r"$\sigma[км/с]$", (2, 37), fontsize=11, style="italic")
    ...
    ax8.set_xlabel("угл. сек.", fontsize=11)
    ...
    # Сохранение с явным указанием всех параметров
    fig.savefig("pgc_35706_model_vs_data.eps", 
                format="eps", 
                dpi=300, 
                bbox_inches='tight',
                pad_inches=0.05,
                ...
                edgecolor='white',
                transparent=False,
                metadata={'Creator': 'Matplotlib', 'Producer': 'Matplotlib EPS'})
