...
gal = find_galaxy(ph_gal, binning= 6, fraction=0.2, level=None,
                nblob = 1, plot=True, quiet=False)
fig.savefig(path + '/' + f'{GAL_NAME}_{BAND}_find',format = "pdf")
gal.pa = 45
plt.clf()
fig = plt.figure(figsize=(9, 6))
s_gal = sectors_photometry(ph_gal, gal.eps, gal.pa, gal.xpeak, gal.ypeak, minlevel=1, plot=1)
...
mge_gal = mge_fit_sectors(s_gal.radius, s_gal.angle, s_gal.counts, gal.eps,
                        bulge_disk=False, fignum=1, linear=True, negative=False,
                        ngauss=200, normpsf=1., outer_slope=4, plot=True, qbounds=None,
