"""
fig6_leda.py — построение mass_dest (распределение массы по радиусу,
разделённое по λ_z компонентам) и Rad_vs_Lambda_z (2D-распределение
орбит в плоскости R_mean × λ_z) для модели LEDA 2220522.
...
ИСПРАВЛЕНИЯ относительно исходной версии:
  ...
  4. ax.get_xticks() для секундарной оси заменено на явное задание тиков.
  5. Переименовано: mass_polar_ring → mass_spherical (соответствие label'у).
  6. rad для Rad_vs_Lambda_z теперь по возрастанию (как и для mass_dest),
...
M_stars_total = 5.45e9  # [M_sun] — ПОЛНАЯ звёздная масса всей галактики
                        # = Υ × densityStars.totalMass(), вычислено через
                        # compute_dynamical_mass.py (Υ=3.6, MGE+distance из run-script).
                        # ВНИМАНИЕ: это масса всей галактики, не только в IFU.
                        # Сумма орбит в IFU даст M_stars_IFU ≈ 0.49×10^10 M_sun
                        # (см. ниже), что согласуется с табличной M_dyn=0.5×10^10.
Max_rad = 10  # [угл. сек.]
...
try:
    _bin_scheme = np.loadtxt("bins_LEDA_2220522_Damir's.txt")
    _r_bin = np.sqrt(_bin_scheme[:, 0]**2 + _bin_scheme[:, 1]**2)
    IFU_RADIUS_ARCSEC = _r_bin.max() * 0.5
    print(f"Радиус MaNGA IFU = {IFU_RADIUS_ARCSEC:.2f} угл. сек. "
          f"(из bins_LEDA_2220522_Damir's.txt)")
except (FileNotFoundError, OSError):
    # Fallback если bin_scheme не доступен
    IFU_RADIUS_ARCSEC = Max_rad
    print(f"[WARN] bins_LEDA_2220522_Damir's.txt не найден, использую Max_rad={Max_rad}")
...
# Параметры halo и BH для расчёта M_dyn в IFU.
# Должны точно совпадать со значениями в run_forstand_grid_LEDA_2220522.py.
RHALO    = 131.0   # core radius лог. halo, в length-units Agama (= arcsec)
VHALO    = 324.0   # асимптотическая v_circ halo, км/с
HALOTYPE = "LOG"   # 'LOG' или 'NFW'
M_BH     = 1e6     # масса BH, M_sun
UPSILON  = 3.6     # M/L (уже учтён в M_stars_total ниже, здесь только для справки)

# Порог |λ_z| для разделения дисковых vs сферических орбит.
# Должен совпадать с разбиением, использованным в wirb_LEDA_clean.py.
...
# archive["Rmean"] хранится в кпк (см. schwarzlib.py:964 — Rm считается напрямую
# из t[:,0:3] в Agama-единицах, kpc). В оригинальном fig6_leda.py деление
# на R_scale было ПРОПУЩЕНО — баг (а в fig6_pgc.py было). Восстанавливаю.
...
# Бин k имеет центр rad_centres[k], границы [rad_centres[k]-zone, rad_centres[k]+zone].
# Так как rad_centres равномерны, индекс бина: round(R / step_size).
...
# Y-предел подбирается автоматически:
...
ax.annotate(r"LEDA 2220522", (0.95, 0.95), xycoords='axes fraction',
...
fig.savefig("mass_dest.pdf", format="pdf", bbox_inches='tight')
plt.close(fig)
print("Сохранено mass_dest.pdf")
...
orb_weights_dist(orbits,
                 ...
                 steps=21,
                 plot_filename="Rad_vs_Lambda_z.pdf",
