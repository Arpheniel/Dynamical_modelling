...
spectro = fits.open("results_8254-1902_vorb020_md19_ad-1_nmom4.fits")  # open MANGA results of analysis
    ...
    for y in range(0,32):
        for Bin_i in range(0,Len_bin):
            if Bin_map[x][y] == Bin_i:
                BINS.append([x-16,y-16,Bin_i])
                VEL_BIN[Bin_i] = Vel_map[x][y]-LOF_vel
...
nump_data = np.transpose([VEL_BIN,VEL_ERR_BIN,SIG_BIN,SIG_ERR_BIN,H3_BIN,H3_ERR_BIN,H4_BIN,H4_ERR_BIN])
