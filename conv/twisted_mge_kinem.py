...
file = fits.open(PHOT)  # open legacy cutoff
ph_gal = file[0].data[2]
...
gal = find_galaxy(ph_gal, binning= 25, fraction=0.3, level=None,
                nblob = 0, plot=True, quiet=False)
fig.savefig(path + '/' + f'{GAL_NAME}_{BAND}_find',format = "pdf")
plt.clf()
print(gal.pa)
#gal.pa = 50
fig = plt.figure(figsize=(9, 6))
mask = dist_circle(gal.xpeak,gal.ypeak,[50,50])
s_gal = sectors_photometry_twist(ph_gal, gal.pa, gal.xpeak, gal.ypeak, minlevel=0, plot=1,mask=mask)
fig.savefig(path + '/' + f'{GAL_NAME}_{BAND}_secphot',format = "pdf")

mge_gal = mge_fit_sectors_twist(s_gal.radius, s_gal.angle, s_gal.counts, gal.eps,
                        fignum=1, negative=False,
...
total_counts, sigma, q_obs, gauss_pa = mge_gal.sol  # assign the solution to variables
...
fig = plt.figure(figsize=(9, 6))
mge_contours = mge_print_contours_twist(ph_gal, gal.pa, gal.xpeak, gal.ypeak, mge_gal.sol, scale=SCALE,
...
for i in range(x1):
    for j in range(y1):
        X_pix.append((i - x1/2)*0.5)
        Y_pix.append((j - y1/2)*0.5)
